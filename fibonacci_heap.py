...
class FibonacciHeap(Heap):
    ...
    def merge(self, fibonnaci_heap) -> None:
        ...
        Fusionne deux arbres
        """
        if type(fibonnaci_heap) != (FibonacciHeap or object):
        ...
        if FibonacciHeap.find_min(self) < FibonacciHeap.find_min(fibonnaci_heap):
            for tree in fibonnaci_heap.trees:
                for i in range(0, len(tree.value)):
                    FibonacciHeap.insert(self, tree.value[i])
        else:
            for tree in self.trees:
                for i in range(0, len(tree.value)):
                    FibonacciHeap.insert(fibonnaci_heap, tree.value[i])
